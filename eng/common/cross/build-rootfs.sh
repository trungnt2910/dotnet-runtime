...
if [[ "$__CodeName" == "alpine" ]]; then
    ...
    git clone https://github.com/haiku/haiku-toolchains-ubuntu --depth 1
    wget -O "$__RootfsDir/tmp/download/hosttools.zip" $(haiku-tools/fetch.sh --hosttools)
    unzip -o "$__RootfsDir/tmp/download/hosttools.zip" -d "$__RootfsDir/tmp"
    ...
    for file in "$__RootfsDir/tmp/download/"*.hpkg; do
        ...
        LD_LIBRARY_PATH="$__RootfsDir/tmp" "$__RootfsDir/tmp/package" extract -C "$__RootfsDir/boot/system" "$file"
    done

    # Download buildtools
    echo "Downloading Haiku buildtools"
    wget -O "$__RootfsDir/tmp/download/buildtools.zip" $(haiku-tools/fetch.sh --buildtools --arch=$__HaikuArch)
